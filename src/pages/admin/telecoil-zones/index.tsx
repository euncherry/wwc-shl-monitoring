...
/* ══════════════════════════════════════════════════════
   Register Modal — 통합 생성 (POST /zones → POST /users 실연동 오케스트레이션.
   단일 트랜잭션 엔드포인트가 없어 계정 생성만 실패하면 존은 남는다 — 부분 성공 안내로 처리)
   ══════════════════════════════════════════════════════ */
      ...
      <div className="flex max-h-[88vh] w-full max-w-lg flex-col overflow-hidden rounded-2xl border border-border bg-white shadow-2xl" onClick={(e) => e.stopPropagation()}>
        <div className="flex shrink-0 items-center justify-between border-b border-border bg-page/50 px-6 py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="flex items-center text-lg font-bold text-foreground">텔레코일존 등록</h3>
        ...
        {isLoading || !zone ? (
          ...
          <>
            ...
            <div className="flex-1 space-y-6 overflow-y-auto scrollbar-thin px-6 py-5">
              ...
              {/* 담당자 이메일 = 계정(zone.user) email — PATCH /users/:id 실연동. 계정 없으면 수정 불가 */}
                    ...
                    {devices.map((d) => (
                      <div key={d.id} onClick={() => setSelectedDevice(d)} title="기기 상세 보기" className="cursor-pointer rounded-xl border border-border bg-page/30 p-4 transition-colors hover:border-primary/40 hover:bg-primary/5">
                        ...
                        <div className="grid grid-cols-4 gap-2">
                          ...
                          <div className="flex flex-col items-center gap-1 rounded-lg border border-border/30 bg-white/60 py-2"><Thermometer className={`h-3.5 w-3.5 ${!d.power ? 'text-muted-foreground/50' : d.overTemperature ? 'text-destructive' : 'text-success'}`} /><span className={`text-[10px] font-bold ${!d.power ? 'text-muted-foreground' : d.overTemperature ? 'text-destructive' : 'text-muted-foreground'}`}>{!d.power ? '—' : d.overTemperature ? '과열' : '정상'}</span></div>
            ...
            <div className="flex shrink-0 flex-wrap items-center justify-between gap-2 border-t border-border bg-page/30 px-6 py-4">
              ...
              <button
                ...
                className="group flex items-center justify-center gap-2.5 rounded-xl border border-transparent py-2 pl-3 pr-4 text-[13px] font-bold text-primary-dark shadow-sm transition-all hover:shadow-[0_4px_16px_rgba(36,107,209,0.15)] disabled:opacity-40"
              ...
              {filteredZones.map((zone) => {
                ...
                return (
                  <div key={zone.id} className={`rounded-xl border ${borderAccent} group cursor-pointer bg-white p-5 transition-all hover:-translate-y-0.5 hover:shadow-lg`} onClick={() => setSelectedZoneId(Number(zone.id))}>
                    ...
                    {/* 통계 — 전체장비 / 정상가동(connection_status===ONLINE 파생) 모두 실값 */}
